                ...
                <div id="solgovde">
                    <div class="sblock">
                        ...
                        <div class="dizi-detay"
                            ...
                            ?>
                            <div id="alt-butonlar">
                                <a href="agonder.php?diziname=Death Note">Altyazı Gönder</a>
                                <?php
                                $k_adi = $_SESSION["Kullanici"];
                            ...
                            if(isset($_GET["altsecenek"])){
                                ...
                                elseif($_GET["altsecenek"] == "leklecik"){
                                    $liste_ekle_kaldir_sorgu = $db->query("DELETE from izlemeliste WHERE userid = '$uye_id' AND diziadi = 'Death Note'");
                                    header("Location:../film/death.php");
                                }
                                else{
                                    header("Location:../film/death.php");
                    ...
                    <div id="dizi-yorumlari"
                        ...
                        if(isset($_POST["comments"])){
                            ...
                            $yorumekleme_sorgusu = $db->query("INSERT INTO yorumlar (username,yorum,diziadi,begeni,dizibolum,dizisezon) values('$yorumyapan_adi','$comment','Death Note',0,'$dizi_bolum',' $dizi_sezon')");
                        ...
                        $sayfa_yorum_yazdirma_sorgusu = $db->query("SELECT * FROM yorumlar WHERE diziadi='Death Note' ORDER BY id DESC LIMIT $sayfa,5");
                        ...
                        if($sayfa_yorum_yazdirma_kontrol > 0){
                                ...
                                while(@$altyorum_verileri = $altyorumbul_sorgu->fetch_assoc()){
                                    ...
                                    if(@isset($_SESSION["Kullanici"])){ 
                                        ...
                                        if($begeni_sorgu_kontrol > 0){
                                            echo'
                                        <a href="../film/death.php?sayfa=yorum&like2=1&yorum='.$alt_yorum_id.'&yorums='.$sayfa.'"><span class="unlike-butonu"></span></a> 
                                       
                                        ';
                                        }
                                        else{
                                            echo'
                                        <a href="../film/death.php?sayfa=yorum&like2=1&yorum='.$alt_yorum_id.'&yorums='.$sayfa.'"><span class="like-butonu"></span></a>
                                        ';
                                        }
                                    }
                                    else{
                            ...
                            if(@isset($_SESSION["Kullanici"])){ 
                                ...
                                if($begeni_sorgu_kontrol > 0){
                                    echo'
                                <a href="../film/death.php?sayfa=yorum&like=1&yorum='.$yorum_id.'&yorums='.$sayfa.'"><span class="unlike-butonu"></span></a> 
                                ';
                                }
                                else{
                                    echo'
                    ...
                    <?php
                        ...
                        $subs_sorgu = $db->query("SELECT * FROM subs WHERE diziname='Death Note' AND onay = '1' ");
                        $subs_sorgu_kontrol = $subs_sorgu->num_rows;
                        $sezonlar = array();
                        if($subs_sorgu_kontrol > 0){
                            $sezon_kontrol = $db->query("SELECT DISTINCT sezon FROM subs WHERE diziname='Death Note' AND onay = '1' ORDER BY sezon DESC");
                            while($deger = $sezon_kontrol->fetch_assoc()){
                                array_push($sezonlar,$deger["sezon"]);
                            }
                            for($i = 0 ;$i<count($sezonlar);$i++){
                                $s = $sezonlar[$i];
                                $altyazı_bul = $db->query("SELECT * FROM subs WHERE sezon = '$s' AND onay = 1 AND diziname='Death Note' ORDER BY bolum ");
                                ?>
