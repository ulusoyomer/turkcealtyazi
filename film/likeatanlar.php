    ...
    <div id="cerceve">
        ...
        <div id="govde">
            <div id="zemingovde">
                ...
                <div class="sblock">
                <?php
                if(isset($_GET["likeslist"])){
                    if($_GET["likeslist"] == 1){
                        if(isset($_GET["yorumid"])){
                            $yorum_id = filtre($_GET["yorumid"]);
                            $yorum_id_sorgu = $db->query("SELECT * FROM ybegeni WHERE yid = '$yorum_id'");
                            $yorum_id_kontrol = $yorum_id_sorgu->num_rows;
                            if($yorum_id_kontrol > 0){
                                while($yorum_veri = $yorum_id_sorgu->fetch_assoc()){
                                    $user_name = $yorum_veri["username"];
                                    $user_name_id_sorgu = $db->query("SELECT * FROM uyeler where username = '$user_name'");
                                    $username_veri = $user_name_id_sorgu->fetch_assoc();
                                    $username_id = $username_veri["id"];
                                    echo '
                                        <a href="../members/'.$username_id.'.php" style="color:#4285f4;display:inline-block;">
                                                '.$user_name.', 
                                        </a>
                    ...
                    else if($_GET["likeslist"] == 2){
                        if(isset($_GET["yorumid"])){
                            $yorum_id = filtre($_GET["yorumid"]);
                            $yorum_id_sorgu = $db->query("SELECT * FROM aybegeni WHERE ayid = '$yorum_id'");
                            $yorum_id_kontrol = $yorum_id_sorgu->num_rows;
                            if($yorum_id_kontrol > 0){
                                while($yorum_veri = $yorum_id_sorgu->fetch_assoc()){
                                    $user_name = $yorum_veri["username"];
